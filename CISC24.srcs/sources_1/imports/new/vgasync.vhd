...
constant VFP	: integer range 0 to 521 := TdispV + TfpV; --490
constant VSP	: integer range 0 to 521 := TdispV + TfpV + TpwV; --492
...
begin
    ...
    begin
      if (rising_edge(PIXEL_CLK) and ven='1') then
                if(rst = '1') then
                    vcounter <=0;
    ...
    do_hs: process(hcounter)
    begin
        if( hcounter>128 ) then
            HS <= SPP;
        else
            HS <= not SPP;
        end if;
